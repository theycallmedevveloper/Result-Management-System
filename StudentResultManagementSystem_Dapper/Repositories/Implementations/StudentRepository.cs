...
namespace StudentResultManagementSystem_Dapper.Repositories.Implementations
{
    public class StudentRepository : IStudentRepository
    {
        ...
        SELECT 
            ...
            SUM(CASE WHEN sr.Status = 'Pass' THEN 1 ELSE 0 END) AS PassStudents,
            SUM(CASE WHEN sr.Status = 'Fail' THEN 1 ELSE 0 END) AS FailStudents
        FROM Students s
        LEFT JOIN StudentResultView sr ON s.StudentId = sr.StudentId
    ";

            return db.QuerySingle<AdminDashboardStatsDto>(sql);
        }
