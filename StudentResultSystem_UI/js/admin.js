...
function loadAdminProfile() {
    // Static for now – later can be JWT based
    document.getElementById("adminName").innerText = "Admin";
    document.getElementById("adminId").innerText = "ID: 1";
}

function loadDashboardStats() {
    fetch("https://localhost:7240/api/auth/admin/dashboard-stats")
        .then(res => res.json())
        .then(data => {
            document.getElementById("totalStudents").innerText = data.totalStudents;
            document.getElementById("passStudents").innerText = data.passStudents;
            document.getElementById("failStudents").innerText = data.failStudents;
        })
